...
export function AiSummaryPane({ link, onOpenSettings }: AiSummaryPaneProps) {
  ...
  const SCREENSHOT_SENTINEL = '{{PAGE_SCREENSHOT}}';
  const summaryParts = link.aiSummary?.split(SCREENSHOT_SENTINEL);
  const hasSentinel = summaryParts !== undefined && summaryParts.length === 2;
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-1 overflow-y-auto p-4">
        {hasSentinel && link.screenshotUrl ? (
          <>
            ...
            <img
              ...
              alt="Page screenshot"
              className="w-full object-cover max-h-[280px] rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm my-4"
            />
            <ReactMarkdown remarkPlugins={[remarkGfm]} components={markdownComponents}>
              {summaryParts![1]}
            </ReactMarkdown>
          ...
          <ReactMarkdown remarkPlugins={[remarkGfm]} components={markdownComponents}>
            {hasSentinel ? link.aiSummary!.replace(SCREENSHOT_SENTINEL, '') : link.aiSummary}
          </ReactMarkdown>
        )}
      </div>
