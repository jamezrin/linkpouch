...
public class RedisEventPublisher implements EventPublisher {
    ...
    public void publishLinkAdded(LinkAddedEvent event) {
        try {
            ...
            RecordId recordId = redisTemplate.opsForStream()
                    .add(MapRecord.create(LINK_STREAM_KEY, eventData));
            
            log.info("Published link.added event to Redis Stream: linkId={}, recordId={}", 
                    event.linkId(), recordId);
        } catch (Exception e) {
    ...
    public void publishScreenshotRefreshRequested(ScreenshotRefreshEvent event) {
        try {
            ...
            log.info("Published screenshot.refresh.requested event to Redis Stream: linkId={}, recordId={}", 
                    event.linkId(), recordId);
        } catch (Exception e) {
